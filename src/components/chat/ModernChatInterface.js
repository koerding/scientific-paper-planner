...
const ModernChatInterface = ({
  ...
  const chatLoading = useAppStore((state) => state.loadingStates.chat);
  ...
  // Send message wrapper with loading state management
  const handleSendMessageWithTracking = async () => {
    if (!currentSection || chatLoading || currentMessage.trim() === '') return;
    ...
    try {
      // Call the handleSendMessage function (now assumed to be async)
      await handleSendMessage();
    } finally {
      // Clear loading state when done (whether success or error)
  ...
  return (
    <>
      ...
      {isMinimized && (
        <div
          className={`fixed bottom-6 right-6 z-50 ${chatLoading ? 'cursor-wait' : 'cursor-pointer'} ${showChatHighlight ? 'onboarding-highlight-chat' : ''}`}
          style={{ transform: 'translateZ(0)' }}
        >
          <button
            onClick={chatLoading ? null : toggleChat}
            disabled={chatLoading}
            className={`flex items-center justify-center px-4 py-3 rounded-full shadow-lg transition-colors text-white font-medium ${chatLoading ? 'bg-indigo-400 cursor-wait' : 'bg-indigo-600 hover:bg-indigo-700'}`}
