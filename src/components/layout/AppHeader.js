...
// MODIFIED: Reduced height to 40px and adjusted internal spacing
...
const AppHeader = ({
  ...
  onOpenReviewModal,
  // REMOVED: setShowExamplesDialog prop is no longer needed
  showHelpSplash,
}) => {
  ...
  return (
    <header className="bg-white shadow-sm fixed top-0 left-0 right-0 z-50 h-10"> {/* UPDATED: Reduced height to 40px (h-10) */}
      <div className="container mx-auto px-4 h-full flex items-center"> {/* UPDATED: Using full height */}
        {/* --- justify-between ensures left/right alignment --- */}
        <div className="flex items-center justify-between w-full">
          {/* Logo (Left) */}
          <div className="flex items-center">
            <div className="w-6 h-6 bg-purple-600 text-white rounded-md flex items-center justify-center mr-2"> {/* UPDATED: Reduced logo size */}
              <span className="font-bold text-sm">SP</span> {/* UPDATED: Smaller text */}
            </div>
          ...
          <div className="flex items-center space-x-1">
            ...
            <button onClick={handleNewButtonClick} disabled={isAiBusy || localImportLoading} className={getButtonClasses()}>
              {isAiBusy ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg> )} New
            </button>
            {/* Import */}
             <label className={getImportButtonClasses()} style={(isAiBusy || localImportLoading) ? { pointerEvents: 'none' } : {}}>
               {localImportLoading ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" /></svg> )} {localImportLoading ? "Importing..." : "Pdf->Example"}
            ...
            <button onClick={saveProject} disabled={isAiBusy || localImportLoading} className={getButtonClasses()}>
               {isAiBusy ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" /></svg> )} Save
            </button>
            {/* Load */}
             <label className={getButtonClasses()} style={(isAiBusy || localImportLoading) ? { pointerEvents: 'none' } : {}}>
               {isAiBusy ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg> )} Load
               <input type="file" className="hidden" accept=".json" onChange={handleFileSelection} disabled={isAiBusy || localImportLoading} />
            </label>

            {/* --- EXAMPLES BUTTON REMOVED --- */}
            ...
            <button onClick={exportProject} disabled={isAiBusy || localImportLoading} className={getButtonClasses()}>
               {isAiBusy ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg> )} Export
            </button>
            {/* Pro Mode */}
            <ProModeToggle />
            {/* Review */}
            <button onClick={onOpenReviewModal} disabled={isAiBusy || localImportLoading} className={getReviewButtonClasses()}>
               {isAiBusy ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> )} Review
            </button>
            {/* Help */}
            <button onClick={handleHelpClick} disabled={isAiBusy || localImportLoading} className={getButtonClasses()}>
              {isAiBusy ? loadingSpinner : ( /* SVG */ <svg className="h-3 w-3 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> )} Help
            </button>
          </div>
