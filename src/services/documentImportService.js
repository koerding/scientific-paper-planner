 ...
 * REFACTORED: Uses centralized prompt content.
...
export const importDocumentContent = async (file) => { //
  ...
  try {
    ...
    // Step 2: Build system prompt using our utility
    const systemPrompt = buildSystemPrompt('documentImport', {
      needsResearchContext: true, // Always include research approach context
      documentText: documentText
    });

    // Step 3: Prepare the main prompt for OpenAI using extracted text
    const mainPrompt = `
...
Based *only* on the text in the document, create the scientific paper structure in the specified JSON format.
`;
